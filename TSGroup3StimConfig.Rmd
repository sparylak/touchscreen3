...
dfkeep <- data.frame(date = {}, mouse = {}, schedule = {}, correctgrid = {}, samplegrid = {}, correct = {}, incorrect = {})
...
for(fil in filestouse){
    ...
    trackerdf <- data.frame(date = rep(sessiondate,dfrows), mouse = rep(mouseid,dfrows), schedule = rep(sessionsched,dfrows), correctgrid = rep(0,dfrows), samplegrid = 0, correct = 0, incorrect = 0, row.names = (1:dfrows))
...
combotabs <- ddply(totalskeep, .(correctgrid, samplegrid), summarize, totaltrials = sum(trialcount), correct = sum(correcttotal), incorrect = sum(incorrecttotal), meanalldays = (correct/(correct+incorrect)))
print(combotabs)
...
###Correct 3, Sample 1:
print(chisq.test(combotabs[5,4:5]))
###Correct 3, Sample 2:
print(chisq.test(combotabs[6,4:5]))
```

**Check sample vs. choice phase separately.  Choice first**

```{r}
###Ignore sample, check choice
combotabs <- ddply(totalskeep, .(correctgrid), summarize, totaltrials = sum(trialcount), correct = sum(correcttotal), incorrect = sum(incorrecttotal), meanalldays = (correct/(correct+incorrect)))
print(combotabs)
print(chisq.test(combotabs[,3:4]))

###Correct grid at position 1
print(chisq.test(combotabs[1,3:4]))
###Correct grid at position 2
print(chisq.test(combotabs[2,3:4]))
###Correct grid at position 3
print(chisq.test(combotabs[3,3:4]))
```

**Now sample.**

```{r}
###Ignore choice, check sample
combotabs <- ddply(totalskeep, .(samplegrid), summarize, totaltrials = sum(trialcount), correct = sum(correcttotal), incorrect = sum(incorrecttotal), meanalldays = (correct/(correct+incorrect)))
print(combotabs)
print(chisq.test(combotabs[,3:4]))

###Sample at position 1
print(chisq.test(combotabs[1,3:4]))
###Sample at position 2
print(chisq.test(combotabs[2,3:4]))
###Sample at position 3
print(chisq.test(combotabs[3,3:4]))
```

**Note that for Match mice, the Correct_Grid_Position actually indicates, the foil, and the Incorrect_Grid_Position is the blank.  So the comma notation refers to the following: (ImageAppearingInChoiceOnly,ImageAppearingInSample)**

```{r fig.height = 8}
###Output graph of percent correct across days for all stimulus combinations split by mouse
bymouse <- ddply(totalskeep, .(mouse, schedule, correctgrid, samplegrid), summarize, totaltrials = sum(trialcount), correct = sum(correcttotal), incorrect = sum(incorrecttotal), meanalldays = (correct/(correct+incorrect)))
bymouse$trialcode <- paste(bymouse$correctgrid,bymouse$samplegrid, sep = ",")
bymouse$trialcode <- factor(bymouse$trialcode, levels = c("1,2","1,3","2,1","2,3","3,1","3,2"))
library(ggplot2)
g <- ggplot(bymouse, aes(x = trialcode, y = meanalldays)) + geom_point(aes(color = schedule),size = 4) + coord_cartesian(ylim = c(0,1)) + facet_wrap(~ mouse, ncol = 3)
print(g)
```
...
hist(dfdoubleinc$incorrect, breaks = 20)
```