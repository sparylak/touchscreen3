...
datestouse <- unique(as.Date(gsub("Mouse[1-12].csv", "", filestouse), format = "%Y%m%d"))
...
for(fil in filestouse){
    ...
    ms1 <- read.csv(file = paste("raw/",fil,sep=""), header = TRUE, stringsAsFactors = FALSE)
    numrows <- length(ms1$Item_Name)
    mouseid <- as.integer(gsub("^[0-9]*(Mouse)|.csv","",fil))
    sessiondate <- as.Date(gsub("Mouse[1-12].csv","", fil), format="%Y%m%d")
